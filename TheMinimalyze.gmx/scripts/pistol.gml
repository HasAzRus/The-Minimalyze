///Pistol
    ...
    if key_fire && !grabbed && ammo > 0
    {
        ...
        {
            damage = 2;
            motion_add(other.weap_dir, 6 * other.alt_fire_spd);
    ...
    if key_weap_drop
    {
        ...
        {
            set_weapon = 0;
            used_weap = instance_create(x, y, o_pistol_used);
            with(used_weap)
            {
                throwed = true;
        ...
        {
            set_weapon = 0;
            weap = instance_create(x - lengthdir_x(weap_drop_distance, weap_dir), y - lengthdir_y(weap_drop_distance, weap_dir), o_pistol);
            with(weap)
            {
                ammo = other.ammo;
