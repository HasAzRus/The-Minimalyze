///MP5
    ...
    if key_alt_fire && !grabbed && ammo > 0
    {
        ...
        {
            damage = 2;
            motion_add(other.weap_dir - choose(-2, 2), 4);
    ...
    if key_weap_drop
    {
        ...
        {
            set_weapon = 0;
            used_weap = instance_create(x, y, o_mp5_used);
            with(used_weap)
            {
                throwed = true;
        ...
        {
            set_weapon = 0;
            weap = instance_create(x - lengthdir_x(weap_drop_distance, weap_dir), y - lengthdir_y(weap_drop_distance, weap_dir), o_mp5);
            with(weap)
            {
                ammo = other.ammo;
