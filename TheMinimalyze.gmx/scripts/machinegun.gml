///Machinegun    
    ...
    if key_alt_fire && !grabbed && ammo > 0
    {
        ...
        bullet = instance_create(x + lengthdir_x(6, weap_dir), y + 1 + lengthdir_y(6, weap_dir), o_bullet);
    ...
    if key_weap_drop
    {
        ...
        {
            set_weapon = 0;
            used_weap = instance_create(x, y, o_machinegun_used);
            with(used_weap)
            {
                throwed = true;
        ...
        {
            set_weapon = 0;
            weap = instance_create(x - lengthdir_x(weap_drop_distance, weap_dir), y - lengthdir_y(weap_drop_distance, weap_dir), o_machinegun);
            with(weap)
            {
                ammo = other.ammo;
